...
const float a1 = 6.5, a2 = 10.5, a3 = 15.5;
const int ANGLE_CORRECTIONS[] = {-5,2, -5};
...
void moveToPosition(float x, float y, float z) {
  ...
  smoothMove(servo2, mapAngle(angles[1] + ANGLE_CORRECTIONS[1], 0, 180, 0, 160));
  smoothMove(servo3, mapAngle(angles[2] + ANGLE_CORRECTIONS[2], 0, 180, 0, 156));
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (radius >= 5.65 && radius < 7.08) {
      moveToPosition(x, y, -2.6);
    }
    else if (radius >= 7.10 && radius < 9.92) {
      moveToPosition(x, y, -2.6);
